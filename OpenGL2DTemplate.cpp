...
void welcomeScreen() {
	...
	const char* message;
	int windowWidth = 800;  // Set your window width here
	float textWidth;

	// Draw top border
	message = "*************************************";
	glColor3f(1, 1, 1); // Set text color to white
	drawText(message, 150, 200, GLUT_BITMAP_TIMES_ROMAN_24);

	// Draw PACMAN centered
	message = "PACMAN";
	textWidth = glutBitmapLength(GLUT_BITMAP_TIMES_ROMAN_24, (const unsigned char*)message);
	drawText(message, (windowWidth - textWidth - 55) / 2, 250, GLUT_BITMAP_TIMES_ROMAN_24);

	// Draw bottom border
	message = "*************************************";
	drawText(message, 150, 300, GLUT_BITMAP_TIMES_ROMAN_24);

	// Draw instructions
	message = "To control Pacman use A to go right, D to go left, W to go up and S to go down.";
	drawText(message, 50, 400, GLUT_BITMAP_HELVETICA_18);

	message = "To start or restart the game, press the space key.";
	drawText(message, 170, 450, GLUT_BITMAP_HELVETICA_18);
